...
) -> Result<ResultVerificationSummary, ToolError> {
    ...
    for item in hand_queries.iter().take(sample_size) {
        ...
        let sqlite_count = match source_action_count(&connection, item) {
            ...
            Err(error) => {
                ...
                push_capped(
                    &mut errors,
                    format!("{context}: source SQLite error: {}", error),
                );
                continue;
            }
        };
        ...
        let binary_count =
            match service.query(&item.dimension(), item.concrete_line_id, &item.hole_cards) {
                ...
                Err(error) => {
                    ...
                    push_capped(
                        &mut errors,
                        format!("{context}: binary query error: {}", error),
                    );
                    continue;
                }
            };
...
fn source_action_count(
    ...
    if statement.step_row()? {
        usize::try_from(statement.column_i64(0)).map_err(|_| {
            ToolError::invalid_format("Source SQLite action count is outside usize range")
        })
    } else {
        Ok(0)
    }
