...
pub fn create_benchmark_workload(
    options: &WorkloadOptions,
) -> Result<BenchmarkWorkload, ToolError> {
...
pub fn read_workload_json(path: &Path) -> Result<BenchmarkWorkload, ToolError> {
    let raw = fs::read_to_string(path)?;
    let parsed: RawBenchmarkWorkload =
        serde_json::from_str(&raw).map_err(|error| ToolError::invalid_format(error.to_string()))?;
...
pub fn write_workload_json(path: &Path, workload: &BenchmarkWorkload) -> Result<(), ToolError> {
...
fn discover_range_dimensions(connection: &Connection) -> Result<Vec<DimensionRef>, ToolError> {
...
fn load_sampling_stats(
    ...
    dimension: &DimensionRef,
) -> Result<SamplingStats, ToolError> {
...
impl<'a> WorkloadSampler<'a> {
    ...
    fn sample_hand_queries(&mut self, count: usize) -> Result<Vec<HandBenchmarkItem>, ToolError> {
    ...
    fn sample_batch_queries(
        ...
        count: usize,
        batch_size: usize,
    ) -> Result<Vec<BatchBenchmarkItem>, ToolError> {
        let mut result = Vec::with_capacity(count);
    ...
    fn sample_abstract_local_hand_queries(
        &mut self,
        count: usize,
    ) -> Result<Vec<HandBenchmarkItem>, ToolError> {
        (0..count)
    ...
    fn sample_stratified_hand_query(&mut self) -> Result<HandBenchmarkItem, ToolError> {
        let stats = self.pick_stats().clone();
    ...
    fn sample_hand_query(&mut self, stats: &SamplingStats) -> Result<HandBenchmarkItem, ToolError> {
        let range_size = stats.max_id.saturating_sub(stats.min_id).saturating_add(1);
    ...
    fn sample_hand_query_by_id(
        ...
        stats: &SamplingStats,
        random_id: u32,
    ) -> Result<HandBenchmarkItem, ToolError> {
    ...
    fn sample_abstract_local_hand_query(&mut self) -> Result<HandBenchmarkItem, ToolError> {
        let stats = self.pick_stats().clone();
        let concrete_ids = self.sample_concrete_ids_for_abstract(&stats)?;
        if concrete_ids.is_empty() {
    ...
    fn sample_range_row_by_id(
        ...
        stats: &SamplingStats,
        random_id: u32,
    ) -> Result<SampledRangeRow, ToolError> {
    ...
    fn sample_abstract_line(&mut self, stats: &SamplingStats) -> Result<String, ToolError> {
        ...
        statement.start(&[Value::from(random_id)])?;
        if statement.step_row()? {
            return statement.column_text(0).map_err(ToolError::from);
        }
        ...
        fallback.start(&[])?;
        if fallback.step_row()? {
            return fallback.column_text(0).map_err(ToolError::from);
        }
        Err(ToolError::invalid_format(format!(
...
fn dimension_key(dimension: &DimensionRef) -> String {
    range_store_core::dimension::dimension_key(dimension)
}
