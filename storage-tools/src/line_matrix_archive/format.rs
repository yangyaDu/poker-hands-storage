...
pub(crate) const DATA_MAGIC: &[u8; 4] = b"LMBN";
pub(crate) const INDEX_MAGIC: &[u8; 4] = b"LMXI";
...
pub(crate) fn read_header(file: &mut File, expected_magic: &[u8; 4]) -> Result<u64, ToolError> {
    ...
    if &header[0..4] != expected_magic {
        return Err(ToolError::invalid_format(
            "LineMatrix archive magic does not match",
        ));
    }
    let version = u16::from_le_bytes([header[4], header[5]]);
    if version != FORMAT_VERSION {
        return Err(ToolError::invalid_format(format!(
            "Unsupported LineMatrix archive format version {version}"
        )));
    }
    let header_size = u16::from_le_bytes([header[6], header[7]]) as usize;
    if header_size != HEADER_SIZE {
        return Err(ToolError::invalid_format(format!(
            "Invalid LineMatrix archive header size {header_size}"
        )));
    }
...
pub(crate) fn read_index_record(
    ...
    let index = concrete_line_id - 1;
    let position =
        (HEADER_SIZE as u64)
            .checked_add(index.checked_mul(INDEX_RECORD_SIZE as u64).ok_or_else(|| {
                ToolError::invalid_format("LineMatrix archive index offset overflow")
            })?)
            .ok_or_else(|| ToolError::invalid_format("LineMatrix archive index offset overflow"))?;
    let mut encoded = [0u8; INDEX_RECORD_SIZE];
