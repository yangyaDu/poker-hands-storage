...
impl HandlePool {
    ...
    pub fn get_or_open(
    ...
    ) -> Result<Arc<DimensionReader>, HandlePoolError> {
        let key = dimension_key_ref(dimension);
        let mut state = self
            .state
            .lock()
            .map_err(|_| HandlePoolError::LockPoisoned)?;
        ...
        let known = self.known.get(&key).ok_or_else(|| {
            HandlePoolError::DimensionNotFound(
...
/// Error from [`HandlePool`] operations.
#[derive(Debug)]
pub enum HandlePoolError {
    LockPoisoned,
    DimensionNotFound(String, u32, u32),
    OpenFailed(String, u32, u32, String),
}

impl std::fmt::Display for HandlePoolError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::LockPoisoned => write!(f, "Dimension handle pool lock poisoned"),
            Self::DimensionNotFound(strategy, player_count, depth_bb) => write!(
                f,
                "Dimension not found: {strategy}:{player_count}max:{depth_bb}BB"
            ),
            Self::OpenFailed(strategy, player_count, depth_bb, error) => write!(
                f,
                "Failed to open dimension {strategy}:{player_count}max:{depth_bb}BB: {error}"
            ),
        }
    }
}

impl std::error::Error for HandlePoolError {}
