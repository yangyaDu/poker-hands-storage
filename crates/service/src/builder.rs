...
use crate::hand_dict::get_hand_id;
use crate::manifest::{BuildManifest, ManifestDimension};
use crate::naming::{
...
pub fn build_store(options: &BuildOptions) -> Result<BuildSummary, AppError> {
    ...
    meta.execute(
        ...
        &[Value::from(source_db_checksum.as_str())],
    )?;
    meta.execute(
    ...
    for dimension in &dimensions {
        manifest_dimensions.push(build_dimension(
            &source,
            &meta,
            &options.out_dir,
            dimension,
...
mod tests {
    ...
    use range_store_core::pack_codec::decode_pack_for_hand;

    #[test]
    fn parses_dimension_name() {
        assert_eq!(
            parse_range_table_name("range_data_default_6max_100BB"),
            Some(DimensionSpec {
        ...
        assert!(parse_range_table_name("range_data_bad").is_none());
    }

    #[test]
    fn encoded_pack_is_readable_by_core_decoder() {
        let rows = vec![
            RangeRow {
                concrete_line_id: 1,
                hole_cards: "AA".to_owned(),
                action_name: "fold".to_owned(),
                action_size: 0.0,
                amount_bb: 0.0,
                frequency: 0.25,
                hand_ev: None,
        ...
        let encoded = encode_concrete_line_pack(&rows).unwrap();
        let cells = decode_pack_for_hand(
            &encoded.payload,
            encoded.hand_count,
            encoded.action_count as u16,
            0,
        );
        assert_eq!(cells.len(), 2);
        assert!((cells[0].frequency - 0.25).abs() < 0.0001);
        assert_eq!(cells[0].hand_ev, None);
        assert!((cells[1].frequency - 0.75).abs() < 0.0001);
    }

    #[test]
    fn utc_date_conversion_matches_epoch() {
        assert_eq!(civil_from_days(0), (1970, 1, 1));
        assert_eq!(civil_from_days(20_629), (2026, 6, 25));
    }
