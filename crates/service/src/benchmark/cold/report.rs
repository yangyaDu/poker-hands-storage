...
pub fn write_cold_start_json(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| AppError::new("IO_ERROR", format!("Cannot create report dir: {e}")))?;
    }
    let json = serde_json::to_string_pretty(report)
        .map_err(|e| AppError::invalid_format(format!("JSON serialize error: {e}")))?;
    fs::write(path, format!("{json}\n"))
        .map_err(|e| AppError::new("IO_ERROR", format!("Cannot write JSON report: {e}")))?;
    Ok(())
}
...
pub fn write_cold_start_markdown(
    ...
    fs::write(path, md)
        .map_err(|e| AppError::new("IO_ERROR", format!("Cannot write Markdown report: {e}")))?;
    Ok(())
}
