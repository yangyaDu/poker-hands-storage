...
use poker_hands_storage_service::benchmark::types::{WorkloadMode, WorkloadOptions};
...
#[test]
fn sqlite_runner_uses_loaded_workload_mode_in_report() {
    let directory = tempdir().unwrap();
    let (source_path, _output_path) = build_verify_fixture(directory.path());
    ...
    let workload = create_benchmark_workload(&WorkloadOptions {
        source_db_path: source_path.clone(),
        requested_dimensions: Vec::new(),
        seed: 11,
        hand_iterations: 3,
        batch_iterations: 2,
    ...
    .unwrap();
    write_workload_json(&workload_path, &workload).unwrap();
    ...
    assert_eq!(report.workload_source.to_string(), "loaded");
    assert_eq!(report.options.workload_mode, WorkloadMode::AbstractLocal);
}
