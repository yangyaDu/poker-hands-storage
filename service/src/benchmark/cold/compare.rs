...
) -> Result<ColdStartCompareReport, AppError> {
...
fn read_cold_report(path: &Path) -> Result<ColdStartBenchmarkReport, AppError> {
    let json = fs::read_to_string(path)?;
    serde_json::from_str(&json).map_err(|error| AppError::invalid_format(error.to_string()))
}
...
fn write_cold_compare_json(path: &Path, report: &ColdStartCompareReport) -> Result<(), AppError> {
