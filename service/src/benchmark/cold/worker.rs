...
/// It opens a fresh QueryService, prewarms one dimension, executes one query,
...
) -> Result<(usize, MemorySnapshot, MemorySnapshot), AppError> {
    // Phase: service open
    let open_start = Instant::now();
    let service =
        QueryService::open_with_meta(params.dir, params.meta, 2, params.verify_checksums)?;
