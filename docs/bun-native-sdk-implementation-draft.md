...
- `storage-tools benchmark-native` 已完成 Rust core、native SDK、HTTP service 的 fair runner，并覆盖 hot、cold、RSS、drill metadata 和 `line-to-hands-by-actions` 单链路。
...
impl NativePokerHandsRange {
    ...
    pub fn hands_by_actions(
        ...
        // idx lookup -> bin decode -> action/frequency filter
    }
}
```
...
| Rust core baseline | 纯存储核心基准 | 定位 `.idx/.bin` 和 decode 本身性能 |
...
| 阶段 4：native benchmark | 已完成 fair runner + drill + HTTP service 对比 | `storage-tools benchmark-native` 已输出 `core:*`、`native-sdk:*`、`http-service:*` case、drill metadata case、冷启动分解和各 worker 内存观察；HTTP 一致性抽样由 `bun run test:http-consistency` 覆盖 |
...
2. 由 `range-store-native` 提供 Bun native SDK 被测入口。已完成：worker 加载 `range-store-native/index.js`。
3. 对比 SQLite local、Rust core baseline、Bun native SDK、Rust HTTP binary。已完成：`benchmark-native` 复用同一个 workload JSON，并把 core、native-sdk、http-service 拆到独立 worker/service 执行。
...
| `--workload` / `--write-workload` | 读取或写出共享 workload，便于和 SQLite / Rust core baseline 使用同一批查询 |
