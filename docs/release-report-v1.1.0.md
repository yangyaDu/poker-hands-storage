...
- `meta.db`：SQLite 文件，包含 build_info、action_schemas、dimension_action_schemas、concrete_lines_{dimension}、drill_scenario_lines 等表
...
| meta.db 维度表 | 每个维度的 concrete_lines_{dim} 表和 drill_scenario_lines_{strategy} 表存在 | MISSING_TABLE |
...
| 索引-包大小 | byte_length == hand_count * (5 + action_count * 8) | PACK_SIZE_MISMATCH |
