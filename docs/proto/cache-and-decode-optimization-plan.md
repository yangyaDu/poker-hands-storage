...
状态：阶段 1 的参数化观测、带可复现实验指纹的 source-derived traversal 和开发态 canonical replay 输入已实现；正式可比基准待实现
更新日期：2026-07-16
...
- 任何会改变生产 cache/prewarm/decoder 行为的候选方案，必须同时报告延迟、RSS/估算缓存字节和
  Core/SQLite 全量策略值一致性，不能只比较结果数。source-derived traversal 仅用于诊断 locality；
  在正式 replay 基线完成前，不得单独作为上线或算法选择依据。
...
每次正式基线都记录：机器、构建 profile、`verify_checksums`、matrix cache 容量、byte budget、
max open handles、workload 文件 hash、样本数和 process-cold/page-cache 限制。source-derived
traversal 没有独立 workload 文件时，必须记录 source SQLite/Proto manifest 指纹、起点、session
数与叶子选择算法版本，不能只保留汇总表。
...
| `matrix_revisit_miss` | benchmark workload observer | 之前访问过、但本次未命中；它是诊断候选，**不单独说明**由 matrix LRU 淘汰造成。当前 JSON 的遗留字段名为 `matrix_revisit_after_eviction_misses`，在补齐原因分类前按此语义解读。 |
| `matrix_lru_eviction` / `matrix_oversized_skip` | `SimpleLru` | 已发生的 matrix LRU 淘汰数，以及因 entry/byte 约束未入 cache 的次数；二者不能混为一类重复 miss。 |
| `dimension_handle_open` / `dimension_handle_eviction` | `HandlePool` | handle LRU 的聚合事件；当前不能把某一次 matrix miss 因果归为 handle 淘汰。 |
...
| `resident_estimated_bytes` | `SimpleLru` | 单个 dimension handle 当前/峰值 decoded cache 估算字节。正式多维报告另需 facade 全部驻留 handle 的汇总当前/峰值。 |

“首次访问”和“之前访问过的再次 miss”只能在 benchmark observer 中统计，不应为了这项诊断在
生产 reader 中维护无界的全历史 key 集合。observer 的历史只覆盖当前有限 workload。若要把
再次 miss 归因到 matrix LRU、handle LRU 或 byte budget，reader 只需提供有界的本次访问事件，
不应维护全历史 key 集合。
...
保留一份 `matrixCacheSweep` profile，不与三方 hot 的原始报告混合。entry capacity 与 byte budget
当前均作用于**每个 dimension handle**；正式多维结论必须同时报告每 handle 与 facade 汇总缓存字节，
并把 `max_open_handles` 固定为实验参数。不能把单 handle peak 当作进程的全局 cache budget。
...
| entries | hit rate | 再次 miss | 单 handle peak decoded estimate |
| ---: | ---: | ---: | ---: |
| 4 | 0.0% | 441 | 29.3 KiB |
| 8 | 54.0% | 38 | 44.5 KiB |
| 12 | 59.1% | 0 | 60.3 KiB |
...
此样本的 working set 拐点约为 12 entries；12 以上没有新增 hit。该样本是单 dimension、无 byte
budget 的固定 traversal，因此这里的再次 miss 可解释为该 handle 的 entry-LRU 重读；它仍只能说明
这个 workload 的 locality，不能单独触发 SLRU 或预热决策。child fanout 为 3 到 6，盲目预热会
放大 decode 次数；只有对其他起点和正式 replay 重复、并通过全量策略值校验后，才决定是否做有预算的
child prewarm。原始报告为
`reports/proto-line-transition-9dimensions-summary.md`。

entry count 只用于兼容与对比；生产约束以 byte budget 为主，因为 9max matrix 的已解码大小不均匀。
预算的作用域必须明确为每 handle 或 facade 全局，不能混用。每组输出 hit rate、首次/再次 miss、
matrix LRU eviction、oversized skip、handle 事件、每 handle 与 facade 汇总峰值 resident bytes、RSS、
matrix read P50/P95、Protobuf decode P50/P95 以及端到端 facade P50/P95。

## 九维 F-F 验证

同一 100-session `F-F` traversal 已覆盖全部 `default:6|8|9:100|200|300` 维度。所有候选叶子
均可经隐式 `F` 规范化解析，且没有缺 retained hand 的路径。测试容量范围内，无再次 miss 的最小
...
该模式目前只输出开发观测：它复用三方 runner 的 result-count 检查，但 session profile 本身不具备
正式 replay 所需的 fresh process、固定 warmup、等价 SQLite cache profile 与全量策略值校验。完成
这些 profile 后，才输出可用于决策的 session P50/P95、按原因分类的 miss、每 handle/facade 汇总
decoded matrix 内存和 RSS。它不能与 source 推导的 `--line-transition-start` /
...
| 测量结果 | 后续实现 |
| --- | --- |
| `first_seen_miss` 占主导 | 不修改通用 LRU；评估业务级预热。增大容量不会让首次访问命中。 |
| 已确认的 matrix LRU 淘汰重读显著 | 先选择满足 P95 working set 的 byte budget；只有仍存在 scan pollution 时才评审 SLRU。 |
| `oversized_skip` 或 cache-disabled miss 显著 | 先调整 byte/entry 预算或明确不缓存策略；不得把它归因到 LRU 算法。 |
| 少量热点与大量一次性扫描混合，且已排除 handle/预算原因 | 采用 probation/protected 两段 LRU；新 entry 先进入 probation，第二次命中后再提升。 |
| dimension handle 淘汰主导 | 调整 `max_open_handles` 或按请求 session 固定 dimension，优先于 matrix cache 算法变更。 |

“显著”必须在报告里给出绝对请求数、占比、原因分类和对应 P95 改善，不以单独 hit rate 作为
上线依据。建议只有当**已确认由 matrix LRU 淘汰造成的**重读 miss 至少占该 workload 的 10%，且
候选方案能在约定的 facade RSS/cache budget 下降低 matrix-read P95、并通过全量策略值校验时，才承担
SLRU 的实现复杂度。
...
`PrewarmSummary` 至少返回 requested、deduplicated、loaded、cache_hits、skipped_by_budget、failed、
耗时和估算缓存字节。输入按 `concrete_line_id` 去重，并受 entry/byte budget 双重限制。除非另行设计
独立且可计量的 pinned/prepared cache，预热写入的仍是普通 decoded-matrix LRU，后续请求可以将其淘汰；
不得把普通 LRU 中的预热项表述为保证保留的 prepared cache。
...
1. **进程内 workset prewarm**：将显式 workset 解码后写入现有 `Arc<DecodedCompactLineMatrix>` LRU；
   它不承诺保留到后续请求，是现有设计的直接延伸，不增加磁盘格式。若业务需要保证保留，必须另行
   设计带独立 byte budget、淘汰规则与 RSS 口径的 pinned/prepared cache，不能把它隐含在普通 LRU 中。
...
1. 补齐阶段 1 的 miss 原因分类、facade 汇总缓存统计和 source-derived workload 指纹；保持 observer
   有界，不在生产 reader 维护全历史 key 集合。
2. 实现正式 replay benchmark：四个等价 profile、fresh process、固定 warmup、全量策略值校验和
   每 handle/facade/RSS 内存口径。
3. 在 9 维度的 random/abstract-local/replay workload 上生成正式报告，再选择缓存分支。
4. 若需要，实施 SLRU 或显式 prewarm；不同时实施两者，避免无法归因。
5. 复跑正式 replay、冷启动和三方 hot benchmark；确认所有 Core/Proto/SQLite 全量策略值一致且错误数为零。
6. 仅在 decoder 门槛满足后，单独撰写 sidecar 或 wire decoder 的格式设计与 rollout/rollback 方案。
