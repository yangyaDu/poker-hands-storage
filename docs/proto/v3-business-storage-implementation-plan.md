...
状态：已完成首发发布门禁；V3 是当前主发布格式，HTTP service 和 native SDK 运行时使用 V3
更新日期：2026-07-17
...
这次门禁确认 V3 已可以作为当前主发布格式，service 和 SDK 运行时只读取 V3，既不双读 V2，
也不提供 V2 回退路径。`storage-tools` 的 V3 发布命令使用显式 `v3-` 前缀；CLI 保留 V2 参考命令，
无参数 CLI 不代表选择 V3 运行时。

## 实施定位

V3 是项目首个已完成首发发布门禁的 Proto 业务存储格式，不是从已上线 V2 向 V3 的兼容迁移。
...
- V2 代码已冻结为历史参考：不再推进 V2 功能、优化、benchmark 或兼容方案；任何删除只可另开清理任务。
...
**结果**：V3 达到业务可用性和资源门槛后，成为当前主发布格式；服务和 SDK 的运行时只读取 V3。
...
4. 提供清晰的显式 V3 CLI：单维导出、全部维度导出、standalone verify、SQLite cross verify、benchmark；
   保留 V2 参考命令，但不将其视为发布路径。
5. 更新 README、API 说明、部署输入和运维文档，明确线上只需要 V3 目录，不需要 SQLite 或 V2 产物。
6. 最后再把 service、SDK 配置和公开示例切到 V3 release root；V3 发布始终通过显式 CLI/配置运行。
...
3. 运行 cold/hot benchmark，确认 correctness gate、延迟、cache 上限和 RSS 均符合门槛，并保存报告。
4. 将 service 和 SDK 配置指向新 V3 root 后进行滚动发布；V3 继续作为唯一运行时 Proto 格式。
