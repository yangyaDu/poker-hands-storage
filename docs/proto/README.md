...
本目录同时保留 Proto V2 参考实现文档，以及当前最高优先级的 Proto V3 业务存储实施方案。
...
V3 是计划首个上线的 Proto 业务存储格式。V2 只用于参考 mmap、索引、Protobuf、cache 和 benchmark
实现；V3 不读取 V2，不做 V2/V3 对比，也不提供回退到 V2 的发布路径。

文档与实现冲突时必须修正其中一方，不能另起一套格式说明。

1. [Proto V3 业务存储实施方案](v3-business-storage-implementation-plan.md)：当前最高优先级；定义 drill、action path、hand strategy 三条业务查询链及首发验证。
2. [V2 格式规范](v2-format.md)：V2 参考实现的字段、位图、量化、文件布局和校验。
3. [运行时与查询](runtime-and-query.md)：V2 参考实现的解码索引、缓存、metadata 和 core 接口。
