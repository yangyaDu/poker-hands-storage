...
V3 是首个计划上线且当前代码默认使用的 Proto 业务存储格式。V2 只用于参考 mmap、索引、Protobuf、cache 和 benchmark
实现；V3 不读取 V2，不做 V2/V3 对比，也不提供回退到 V2 的发布路径。

文档与实现冲突时必须修正其中一方，不能另起一套格式说明。

1. [Proto V3 业务存储实施方案](v3-business-storage-implementation-plan.md)：定义 drill、action path、hand strategy 三条业务查询链及首发验证。
