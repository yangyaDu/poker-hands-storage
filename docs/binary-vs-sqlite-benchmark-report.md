...
当前 `benchmark-native` runner 已改为 `core`、`native-direct`、`native-sdk` 三个独立 worker/子进程执行，并可启动独立 HTTP service worker 对比 loopback API。旧 9max 汇总只能作为观察值，不能作为 SDK 架构优劣的定论。

最新公平复测的入口顺序为 `native-sdk, http-service, native-direct, core`，关键 case 如下：

| case | avg | p50 | p95 | QPS | error |
| --- | ---: | ---: | ---: | ---: | ---: |
| `core:hand-strategy` | 0.064 ms | 0.012 ms | 0.374 ms | 15,643 | 0 |
| `native-direct:hand-strategy` | 0.070 ms | 0.013 ms | 0.399 ms | 14,241 | 0 |
...
| `native-direct:batch-hand-strategy` | 0.987 ms | 0.936 ms | 2.150 ms | 1,013 | 0 |
...
| `native-direct:concrete-lines-exact` | 0.398 ms | 0.350 ms | 0.657 ms | 2,515 | 0 |
...
| `native-direct:drill-scenarios-metadata` | 0.016 ms | 0.010 ms | 0.052 ms | 62,625 | 0 |
...
| `native-direct:line-to-hands-by-actions` | 0.436 ms | 0.386 ms | 0.690 ms | 2,294 | 0 |
...
这说明在同 workload 和独立进程口径下，SDK 没有再出现“明显快于 Core”的反常结论；Core、Direct、SDK 基本同阶，小差异不应解释为数据结构差异。HTTP service 稳定慢一档，主要来自 loopback HTTP 和 JSON 序列化边界。`concrete_line -> concrete_line_id` metadata lookup 在走索引后为 sub-ms，但仍明显慢于旧 eager in-memory map 的 0.01ms 左右，换来的是不再把整张 concrete-line 表预加载进 native worker。
...
`reports/benchmark-native-core-direct-sdk-http-9max-indexed-meta.json` 的新 RSS 观察值为：

| Phase | Core | Native Direct | Native SDK |
|---|---:|---:|---:|
| Baseline | 15,908,864 bytes | 138,018,816 bytes | 138,092,544 bytes |
| After import | n/a | 138,682,368 bytes | 150,249,472 bytes |
| After constructor | 16,461,824 bytes | 139,415,552 bytes | 150,982,656 bytes |
| After warmup | 41,005,056 bytes | 171,528,192 bytes | 187,355,136 bytes |
| After benchmark | 99,033,088 bytes | 272,338,944 bytes | 309,649,408 bytes |
| Delta | 83,124,224 bytes | 134,320,128 bytes | 171,556,864 bytes |
