...
> **最新增量复测**: 2026-07-05 已刷新 9max:100BB native fair benchmark、drill metadata microbenchmark、lazy cold-start 对照。
...
| --- | ---: | ---: | ---: | ---: | ---: |
| `core:hand-strategy` | 0.059 ms | 0.010 ms | 0.336 ms | 17,088 | 0 |
| `native-direct:hand-strategy` | 0.059 ms | 0.011 ms | 0.322 ms | 17,027 | 0 |
| `native-sdk:hand-strategy` | 0.094 ms | 0.018 ms | 0.444 ms | 10,657 | 0 |
| `core:batch-hand-strategy` | 0.878 ms | 0.812 ms | 1.972 ms | 1,139 | 0 |
| `native-direct:batch-hand-strategy` | 0.964 ms | 0.902 ms | 2.025 ms | 1,037 | 0 |
| `native-sdk:batch-hand-strategy` | 1.355 ms | 1.321 ms | 2.582 ms | 738 | 0 |
| `core:batch-size-20` | 0.099 ms | 0.084 ms | 0.142 ms | 10,075 | 0 |
| `native-direct:batch-size-20` | 0.136 ms | 0.122 ms | 0.210 ms | 7,356 | 0 |
| `native-sdk:batch-size-20` | 0.187 ms | 0.169 ms | 0.284 ms | 5,356 | 0 |
| `core:concrete-lines-exact` | 0.417 ms | 0.368 ms | 0.651 ms | 2,399 | 0 |
| `native-direct:concrete-lines-exact` | 0.408 ms | 0.366 ms | 0.627 ms | 2,454 | 0 |
| `native-sdk:concrete-lines-exact` | 0.561 ms | 0.523 ms | 0.900 ms | 1,783 | 0 |
| `core:drill-scenarios-metadata` | 0.009 ms | 0.007 ms | 0.026 ms | 114,834 | 0 |
| `native-direct:drill-scenarios-metadata` | 0.022 ms | 0.011 ms | 0.067 ms | 45,555 | 0 |
| `native-sdk:drill-scenarios-metadata` | 0.014 ms | 0.009 ms | 0.038 ms | 71,265 | 0 |
...
### 3.0 最新 9max:100BB lazy cold-start 复测

2026-07-05 已针对 `default:9:100` 重新跑 lazy action schema 之后的 cold-start 对照：

- Binary: `reports/benchmark-cold-start-9max-lazy.json`
- SQLite: `reports/benchmark-sqlite-cold-start-9max-lazy.json`
- Compare: `reports/benchmark-cold-compare-9max-lazy.json`
- 口径：10 runs，`concrete_line_id=1`，`hand=22`，错误数均为 0。

| 指标 | SQLite | Binary | Binary/SQLite |
|---|---:|---:|---:|
| Process elapsed P50 | 47.09 ms | 36.50 ms | 0.78x |
| Process elapsed P95 | 50.34 ms | 43.88 ms | 0.87x |
| Store open + first query P50 | 27.49 ms | 20.53 ms | 0.75x |
| Store open + first query P95 | 31.24 ms | 24.83 ms | 0.79x |
| First query P50 | 16.40 ms | 1.09 ms | 0.07x |
| First query P95 | 20.67 ms | 1.66 ms | 0.08x |

这次结果已经反映 action schema 按 `schema_id` 懒加载后的实现：9max 单维度 cold-start 不再慢于 SQLite，首次策略查询仍明显快于 SQLite。下面 3.1-3.3 是 2026-07-01 的全 9 维历史报告，用于观察旧实现和全维度趋势。

以下历史全维度报告使用 `--release` 并固定同一查询口径重跑：
...
|---|---:|---:|---:|
| Baseline | 15,470,592 bytes | 137,994,240 bytes | 138,014,720 bytes |
| After import | n/a | 138,629,120 bytes | 150,020,096 bytes |
| After constructor | 16,089,088 bytes | 139,374,592 bytes | 150,802,432 bytes |
| After warmup | 40,628,224 bytes | 171,503,616 bytes | 187,019,264 bytes |
| After benchmark | 98,721,792 bytes | 273,362,944 bytes | 309,264,384 bytes |
| Delta | 83,251,200 bytes | 135,368,704 bytes | 171,249,664 bytes |

因此当前更准确的说法是：SDK 单 worker 的完整 9max random benchmark RSS 增量约 163.3 MiB，不是旧报告表面上的 407.2 MiB。After-constructor 绝对 RSS 约 143.8 MiB，warmup 后约 178.4 MiB；完整 benchmark 结束后的更高 RSS 还包含 Bun worker workload、SQLite metadata lookup 的页缓存、mmap 工作集和 native/runtime 状态。action schema cache 已改为懒加载，本次公平复测中三组首查后 `schemaCount=1`，完整 benchmark 结束后 `schemaCount=2868`，不再是首次 miss 加载全表。
