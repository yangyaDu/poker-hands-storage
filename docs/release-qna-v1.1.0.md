...
### Q: SDK 的 queryBatch 中单个 item 失败（比如 hand 不存在），为什么要让整个调用失败？

**回答**：batch 查询现在是 all-or-nothing 语义。原因是 native SDK 和 core 都不是 HTTP 聚合层，不能把同一批请求的一部分结果和一部分错误混在一个“成功”响应里返回给业务层。只要某个 item 的参数或数据语义不成立，整个 batch 就抛出同一个错误。
...
for (index, item) in requests.iter().enumerate() {
    let result = self
        .query_single(&reader, dimension, item.concrete_line_id, &item.hole_cards)
        .map_err(|error| StoreQueryError::batch_item(index, item, dimension, error))?;
    results.push(result);
}
```

`query_single` 返回 `StoreQueryError` 后不会被转换成 item-level `error` 字段，而是包装成带 `requests[index]`、`concrete_line_id` 和 `dimension` 的批量错误继续向上抛。
...
HTTP service 仍然保留外层 `{ code, data, message }` envelope；同一个错误会被映射为 HTTP 400/404/500 等状态和公开业务码。成功的 batch 响应只返回 `results: [{ concrete_line_id, hole_cards, actions }]`，不再返回 item-level `error` 或嵌套 `strategy`。
