...
f32 的尾数只有 23 位，有效数字约 7 位十进制。所以 `0.123456789` 截断到 f32 大约是 `0.12345679`（最后一两位丢失），绝对误差在 1e-7 量级。

这里要分清两个字段的业务量纲：

- `frequency` 是概率值，范围是 0..1，绝对误差通常可以直接按概率误差理解。
- `hand_ev` 是以 BB 计的期望值，不是概率值；它可能为负，也可能超过 5 BB，还可能为 null。因此不能用固定的 0..5 BB 区间来概括它的精度影响。

验证统计会按实际源值分别统计 `frequency` 和 `hand_ev` 的量化误差。相对误差使用 `max(|source|, 1.0)` 做分母，是为了避免接近 0 的 EV 把相对误差人为放大。结论不是“hand_ev 一定落在某个固定区间所以可接受”，而是：二进制存储值必须等于源 SQLite REAL 正确转换成 IEEE754 Float32 后的 bit pattern；Float32 格式本身带来的量化误差由统计分布单独呈现。
...
### Q: build-state.json 的 source_checksum 是怎么计算的？如果源 SQLite 被替换或重新导出但业务数据没变，checksum 会变吗？

**回答**：source_checksum 是对源 SQLite 文件的整个文件内容做 checksum（不是对表数据），所以：

- 如果源文件内容完全不变（同一个文件），checksum 不变，resume 可以通过
- 如果换成另一份 SQLite 文件，或者重新 export / 重新生成了一份相同业务数据的 SQLite 文件，只要文件字节不同，checksum 就会变，resume 会拒绝
- 我们不会在构建流程里对源 SQLite 做 `ALTER TABLE` 这类原地 schema 修改；源库按不可变输入处理

这个设计的意图是：source_checksum 检测的是“本次 resume 是否仍然使用同一个源文件字节版本”，而不是“两份源库的业务数据是否语义一致”。更精细的数据校验靠的是 build 过程中计算的每 dimension 的 pack_count 和 concrete_line_count，resume 时会对比这些数字。
...
### Q: CachedMetadataReader 为什么叫 Cached？它缓存的是什么？

**回答**：`CachedMetadataReader` 是运行时 metadata lookup 组件，负责回答两类问题：

- `concrete_line / abstract_line -> concrete_line_id`：查询 `concrete_lines_{strategy}_{N}max_{BB}BB`
- `drill_name + player_count + drill_depth -> abstract_lines`：查询 `drill_scenario_lines_{strategy}`

它叫 Cached，是因为它缓存的是**metadata 查询结果**，不是把整个 `meta.db` 预加载进内存，也不是 SQL statement cache。

构造时只做轻量初始化：读取 manifest 得到可查询的 strategies / dimensions，并打开一个只读 `meta.db` connection。真正的 metadata rows 在第一次访问某个 key 时才查询：

```text
get_concrete_lines(...)
  -> 先查 RwLock<HashMap> 结果缓存
  -> miss 时用 SQLite connection 查询 meta.db
  -> 把结果写入缓存
  -> 后续相同 key 直接走内存
```
...
- `concrete_by_concrete`：`(strategy, player_count, depth_bb, concrete_line) -> ConcreteLineRow`
- `concrete_by_abstract`：`(strategy, player_count, depth_bb, abstract_line) -> Vec<ConcreteLineRow>`
- `drill_lines`：`(strategy, drill_name, player_count, drill_depth) -> Vec<String>`

并发边界也分开处理：

- `RwLock<CachedMetadataState>` 保护内存 HashMap。cache hit 只需要读锁，允许多个查询并发读。
- `Mutex<LockedMetadataConnection>` 保护 SQLite connection。我们的 SQLite connection 用 `SQLITE_OPEN_NOMUTEX`，所以 cache miss 查库时必须串行化。

trade-off 是：缓存会随访问过的 metadata key 增长，但每条 metadata row 很小；换来的收益是重复查询同一 concrete line、abstract line 或 drill key 时，不再反复走 SQLite B-tree lookup。它不改变 `meta.db` 作为 metadata 权威来源的角色，只是在运行时把热点 lookup 结果留在进程内存里。
